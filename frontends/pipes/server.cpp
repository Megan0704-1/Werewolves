...
void PrintServerUsage(std::string_view bin) {
  std::cerr << "Usage: " << bin << " [OPTIONS]\n"
            ...
            << "  --create-fifos     Create pipe tree on startup\n"
            << "\n"
            << "Misc:\n"
...
ServerOptions ParseServerArgs(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (arg == "--create-fifos") {
      options.create_fifos = true;
...
int RunServer(const ServerOptions& options,
  ...
  if (!comm) {
    std::cerr << "Failed to create communication backends for server: pipe\n";
    return 1;
  }
  ...
  std::cout << "backend: pipe" << std::endl;

...
void SignalHandler(int) { running_ref.store(false); }
}  // namespace

void PrintClientUsage(std::string_view bin) {
  std::lock_guard<std::mutex> lock(io_mutex);
  std::cerr << "Usage: " << bin << " <slot-number> [OPTIONS]\n"
            << "\n"
            << "Options:\n"
...
// 2. (if successfully connected) launch a listener thread, keep pooling msg
// from server. (shutdown communication if game closed.)
// 3. another thread getline from the client, send to server if any.
int RunClient(const ClientOptions& options,
              const ClientCommunicationFactory& factory) {
  ...
  if (!comm) {
    std::lock_guard<std::mutex> lock(io_mutex);
    std::cerr << "Failed to create communication backend for client: shm "
                 "(shared memory)\n";
    return 1;
  }

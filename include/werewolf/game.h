...
class Game {
 public:
  explicit Game(std::unique_ptr<IServerCommunication> comm,
                GameConfig cfg = {});
  ~Game();
 ...
 private:
  // nested scope guard
  // This guard ensures proper shutdown when un-expected or abnormal returns
  // before scope ends.
  struct CleanupGuard {
    Game& game;
  ...
  VoteResult choose_night_victim(const std::vector<int>& voters,
                                 const std::vector<int>& slots);
  VoteResult choose_day_target(const std::vector<int>& voters,
                               const std::vector<int>& slots);
  VoteResult collect_votes(const std::vector<int>& voters,
                           const std::vector<int>& cands, int duration);
  ...
  void broadcast_to_slots(const std::string& msg,
                          const std::vector<int>& slots);
  ...
  void log_rounds();
  void log(const std::string& msg, bool to_stdout = true,
           bool to_game_log = true, bool to_moderator_log = true);
};

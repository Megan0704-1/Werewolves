...
TEST(GameTest, WitchCanHealNightVictim) {
    ...
    EXPECT_THAT(log_content, ::testing::Not(HasSubstr("Night: player3 was killed.")));
}
...
TEST(GameTest, WitchSkipKeepsWolfVictimDead) {
    ...
    EXPECT_THAT(log_content, HasSubstr("Final words from player3: I am not a wolf."));
}
...
TEST(GameTest, WitchPoisonKillsPoisonTargetAndKeepsWolfVictimDead) {
    ...
    EXPECT_THAT(log_content, HasSubstr("Final words from player3: goodbye from 3"));
    EXPECT_THAT(log_content, HasSubstr("Final words from player4: goodbye from 4"));
}
